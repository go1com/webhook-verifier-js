...
const createSignature = (timestamp: number, payload: string, secret: string, version: string = 'v1') =>
  `t=${timestamp},${version}=${createHmac('sha256', secret).update(`${timestamp}.${payload}`, 'utf-8').digest('hex')}`;
...
describe('SignatureVerifier', () => {
  ...
  it('finds invalid signature', () => {
    expect(() => {
      verifySignature(createSignature(timestamp, payload, secret), payload, faker.internet.password());
    }).toThrow(InvalidWebhookSignature);
  });

  it('finds invalid timestamp outside default tolerance', () => {
    expect(() => {
      verifySignature(createSignature(timestamp - 61, payload, secret), payload, secret);
    }).toThrow(InvalidWebhookSignatureTimestamp);
  });

  it('finds invalid timestamp version', () => {
    expect(() => {
      verifySignature(createSignature(timestamp, payload, secret, `v${faker.number.bigInt()}`), payload, secret);
...
describe('SignatureVerifier with custom configuration', () => {
  ...
  it('finds invalid timestamp outside configured tolerance', () => {
    configureSignatureVerification({ timestampToleranceInSeconds: 10 });
    expect(() => {
      verifySignature(createSignature(timestamp - 11, payload, secret), payload, secret);
    }).toThrow(InvalidWebhookSignatureTimestamp);
  });
  ...
  it('finds invalid timestamp version', () => {
    ...
    expect(() => {
      verifySignature(createSignature(timestamp, payload, secret, `v1`), payload, secret);
    }).toThrow(InvalidWebhookSignatureVersion);
  });
});
...
describe('isSignatureVerified', () => {
  it('verifies a signature successfully', () => {
    expect(isSignatureVerified(createSignature(timestamp, payload, secret), payload, secret)).toEqual({
      isValid: true,
    });
  });
  ...
  it('finds invalid signature', () => {
    expect(
      isSignatureVerified(createSignature(timestamp, payload, secret), payload, faker.internet.password())
    ).toEqual({
      isValid: false,
  ...
  it('finds invalid timestamp version', () => {
    expect(
      isSignatureVerified(createSignature(timestamp, payload, secret, `v${faker.number.bigInt()}`), payload, secret)
    ).toEqual({
      isValid: false,
